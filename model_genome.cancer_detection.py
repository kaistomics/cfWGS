...
def create_model(num_dense_layers, num_dense_nodes, learning_rate, activation, drop_rate, weight_decay):
    ...
    for i in range(num_dense_layers):
        ...
        x1 = Activation(activation)(x1)
        x1 = Dropout(drop_rate)(x1)
    output = Dense(1, activation="sigmoid", bias_initializer=output_bias)(x1)
...
dim_activation = Categorical(categories=['relu', mish,'tanh','sigmoid'], name='activation')
...
output_file = "./performance_genome.cancer_detection.txt"
...
num_classes = 2
class_weight1 = class_weight.compute_class_weight(class_weight='balanced', classes=np.unique(train_labels), y=train_labels)
...
os.system('mkdir model_genome.cancer_detection')
out = open(output_file, 'w')
for i in range(90,120):
    random.seed(i)
    np.random.seed(i)
    tf.random.set_seed(i)
    final_best_model_path = './model_genome.cancer_detection/%s.seed_%s.h5'%(final_best_model_idx,i)
    model = create_model(num_dense_layers = num_dense_layers,
                         num_dense_nodes = num_dense_nodes,
                         learning_rate=learning_rate,
                         activation=activation,
    ...
    train_loss = min(history.history['loss'])
    vali_loss = min(history.history['val_loss'])
    train_auc, train_confmat = get_result(model, genome_train_data, train_labels)
    vali_auc, vali_confmat = get_result(model, genome_vali_data, vali_labels)
    test_auc, test_confmat = get_result(model, genome_test_data, test_labels)
    out.write("%s"%i + "\t" + str(train_loss) + "\t" + str(vali_loss) + "\t" + str(train_auc) + "\t" + str(vali_auc) + "\t" + str(test_auc) + "\t" + train_confmat + "\t" + vali_confmat + "\t" + test_confmat + "\n")
...
df = pd.read_csv("performance_genome.cancer_detection.txt", sep="\t", header=None, index_col=0)
idx = df.sort_values(by=2).head(1).index.values[0]
os.system("cp ./model_genome.cancer_detection/input_genome.cancer_detection.dnn.loss.best_model.trained.seed_%s.h5 model_genome.cancer_detection.best.h5"%(idx))

os.system("rm input_genome.cancer_detection.dnn.loss.best_model.h5")
os.system("rm input_genome.cancer_detection.dnn.loss.best_model.search_result.summary.pkl")
os.system("rm input_genome.cancer_detection.dnn.loss.tmp_best_model.h5")
os.system("rm -r model_genome.cancer_detection")
